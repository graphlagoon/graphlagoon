...
const community = computed(() => {
  if (props.communityId === null) return null
  return communityStore.communitiesSorted.find(c => c.id === props.communityId) ?? null
})

const nodes = computed(() => {
  if (props.communityId === null) return []
  const nodeIds = communityStore.communitiesById.get(props.communityId)
  if (!nodeIds) return []
  const idSet = new Set(nodeIds)
  return graphStore.nodes.filter(n => idSet.has(n.node_id))
})

const algorithmLabel = computed(() =>
  communityStore.isLouvain ? 'Louvain' : 'Cluster Program'
)
...
function exportCSV() {
  ...
  a.href = url
  a.download = `community-${props.communityId}.csv`
  a.click()
...
<template>
  <div v-if="community" class="modal-overlay" data-testid="community-node-modal" @click.self="handleClose">
