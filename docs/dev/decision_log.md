...
## [2026-08-28 18:00] - Feature Implemented: Custom (user-defined) metrics — sandboxed JS, typed values, writer-only

**Feature:** A user with write access to a context can define new metrics as JavaScript snippets evaluated per node / per edge in a sandboxed Web Worker. Values may be numbers, text or booleans, so a custom metric is also a derived column (regex extraction, concatenation, ratios, structural scores such as neighbour mean degree or clustering coefficient). Results are regular `ComputedMetric`s with the stable id `custom:<definitionId>`, so they reach the Data Table (virtual column typed by value type), node-size / edge-width mapping (numeric only), the side panel / detail modal, and — new for every metric — label templates through `{metric:<name>}`.

**Requirements (from the user):** (1) let users define new metrics safely; (2) also expose them as Data Table columns like the existing virtual columns; (3) not only numeric — text transformations too; (4) only writers/superusers may add them **and only they may see them**.

**Design Decisions:**
1. **JavaScript in a dedicated, non-pooled worker rather than a DSL** (user's choice between the two). Sandbox = scope hardening before any user code (`workers/customMetricSandbox.ts` strips `fetch`, `XMLHttpRequest`, `WebSocket`, `EventSource`, `importScripts`, `Worker`, `SharedWorker`, `BroadcastChannel`, `MessageChannel`/`Port`, `postMessage`, `close`, `indexedDB`, `caches`, `navigator`, … from the scope and its prototype chain; the worker captures its own `postMessage` binding first) + main-thread `terminate()` on a 10 s per-definition timer with respawn for the remaining definitions + frozen inputs + typed coercion in the worker and re-validation on the main thread (`utils/customMetricValue.ts`). Deliberately kept: `Function`/`eval` (scope already stripped; `new Function` is how the code is compiled), `console`, timers.
2. **Visibility is enforced server-side in one place.** `context_to_response` returns `metric_definitions=[]` unless `can_write(...)`; every context endpoint builds its response there. A read-only user therefore never receives — let alone executes — another user's code, which is the strongest guarantee of the design: the code only runs for users who could have written it.
3. **Typed pydantic model, not an opaque blob.** `MetricDefinition` (id pattern, name ≤ 80, code ≤ 20 000, `target`, `value_type`, description ≤ 500) with unique ids and case-insensitive unique names per context; migration 013 (`metric_definitions` JSON column, idempotent inspector guard) + memory-store twin.
4. **`ComputedMetric` generalised** to `values: Map<string, MetricValue>` (`number | string | boolean | null`) with a mandatory `valueType`. `nodeSizeMetric` / `edgeWeightMetric` return `null` for non-numeric metrics so `graphAppearance.ts` keeps its `Map<string, number>` contract (`numericValueMap` narrows at the canvas boundary). New `metricsVersion` counter: the canvas watch keyed on `computedMetrics.size` missed same-size recomputes.
5. **Pure evaluation core** (`workers/customMetricEvaluate.ts`: `buildIndexes` + `evaluateDefinition`) with the worker as a thin shell, so the whole use-case catalogue runs under vitest in Node; the real sandbox is exercised by e2e in Chromium (incl. a hostile definition that tries `fetch`/`importScripts`/`new Worker`/`self.postMessage`/`indexedDB` and must see every call throw).
6. **`ctx` API widened by the use cases:** adjacency (`neighbors`, `out/inNeighbors`, `edgesOf`, `hasEdge`, `isConnected`), `node(id)`, `metrics` / `metric(ref)` (built-in + algorithm runs, latest run per algorithm id — never other custom metrics: no ordering, no cycles), `community(id)`, `graph` summary, full `nodes`/`edges`, and `cache` (per-definition mutable object → global work once, e.g. percentile ranks). Indexes are built once per RUN in the worker; adjacency is not cloned.
7. **Snapshot per recompute cycle, not per definition** (`serializeGraphForCustomMetrics`: `toRaw`, filtered population, degrees, non-custom metrics, communities). Recompute is debounced (750 ms) and coalesced; triggers: hydration, definition edit (that id only), `filteredNodes/Edges.length`, `nodePatchVersion` (late properties).
8. **Persistence follows `contextMenuActions`** (isHydrating guard, 800 ms debounce into one PUT of the full list, no PUT without write access, resync `currentContext`). Hydration drops `custom:*` metrics that are not in the new list (context switch cleanup).
9. **`{metric:<ref>}` label placeholder** resolved through an optional `FormatOptions.metrics` resolver (store's `metricResolver`: id first, then name within target, first match). Existing `formatLabel` callers are untouched and render `[metric:x]`; the canvas, `TemplatePreviewInline` and the Labels panel autocomplete pass the resolver / metric names. Fixes the long-standing gap that no metric was reachable from labels.
10. **Editor** (`CustomMetricEditorModal.vue`, reusing `JavaScriptEditor` with a new `completions` prop): name / target / value type / description / code, **Test on current graph** (first 50 items, values, item errors, raw-type mismatch hint), read-only mode. Custom tab in `MetricsPanel` with per-definition status, table toggle, edit / recompute / delete.
11. **Ask-AI robot button** (`utils/customMetricSkill.ts` + `CustomMetricSkillModal.vue`, wired in the Custom tab and in the editor's code field), mirroring the label-template / actions / cluster-program pattern: pure prompt builder embedding declared ∪ loaded types, properties, the non-custom metric names reachable through `ctx.metrics`, taken custom names, community state, the `item`/`ctx` contract, sandbox rules and worked examples; the LLM is asked to interview first and answer with the editor's JSON shape.
12. **Import / export, same envelope pattern as presets and actions** (`types/portable.ts` `PortableCustomMetrics`, `utils/customMetricImport.ts`). The parser accepts the bare LLM answer (object or array, ```json fence tolerated, `code` as an array of lines), or the envelope; it enforces the server's caps and uniqueness locally so a paste fails in the box, not on the PUT. The editor's Import JSON fills the form (first metric when several were pasted); the Custom tab's Import JSON handles many at once with **upsert semantics** (`store.upsertDefinitions`: match by id, then by name case-insensitively → update in place keeping the id; else add), so re-pasting an edited JSON updates rather than duplicates; in edit mode the editor's box is "Edit as JSON", prefilled with the current definition. Compatibility warnings come from a best-effort static scan of the code (`item.properties.x`, `['x']`, `ctx.metric('x')`, `ctx.metrics['x']`) against the context's properties and the reachable metric refs, and hand off to the robot prompt in **adapt mode** (`customMetricSkill` gained `importedJson`/`importedSource`, reusing `sourceSchemaSection`).
13. **Auto-run is opt-in per definition and gated by two server feature flags.** After reviewing the drive-by risk (a writer's code auto-executing in every other writer's/superuser's browser — contained by the sandbox, but still CPU/memory and consent), `MetricDefinition.auto_run: StrictBool = False`: only flagged definitions evaluate on load / graph change; the rest wait for **Recompute** (status *not computed*, or *stale* once the graph changes under computed values). Author actions (add / edit / Recompute) always evaluate. Following the `style_presets_enabled` / `precomputed_graphs_enabled` precedent, `GRAPH_LAGOON_CUSTOM_METRICS_ENABLED` (off ⇒ definitions hidden from everyone, writes rejected with `400 CUSTOM_METRICS_DISABLED`, Custom tab gone) and `GRAPH_LAGOON_CUSTOM_METRICS_AUTO_RUN_ENABLED` (off ⇒ everything manual, checkbox disabled with the reason) reach the frontend through `window.__GRAPH_LAGOON_CONFIG__` / `GET /api/config`; a new `composables/useFeatureFlags.ts` reads them (absent ⇒ enabled, so older backends behave as before).
14. **`show_in_table` is persisted on the definition.** The 2026-08-26 decision kept `tableMetricIds` session-only because algorithm runs have unstable ids; custom metrics have stable ids and a persisted definition, so their Data Table flag (`MetricDefinition.show_in_table`, StrictBool) lives there — context-level, shared by writers, hydrated into the session set on load, toggled through `store.setShowInTable` (persist, no recompute). Algorithm metrics keep the session toggle.
15. **`stylePresetSkill` told the LLM to use metric ids that do not exist** (`degree`, `in_degree`, `clustering`…). Now lists `__builtin_degree`, `custom:<id>` for the numeric custom metrics of the context, or `null`, and says algorithm runs are session-only.

**Backend Changes:** `db/models.py`, `db/memory_store.py`, `models/schemas.py` (`MetricDefinition`, Create/Update/Response + uniqueness validators), `routers/graph_contexts.py` (visibility in `context_to_response`, create/update DB + memory), `alembic/versions/013_add_context_metric_definitions.py`.

**Frontend Changes:** created `types/customMetrics.ts`, `utils/customMetricValue.ts`, `utils/customMetricCompletions.ts`, `utils/metricFormat.ts`, `composables/useItemMetrics.ts`, `workers/customMetricSandbox.ts`, `workers/customMetricEvaluate.ts`, `workers/customMetricWorker.ts`, `services/customMetricRunner.ts`, `stores/customMetrics.ts`, `utils/customMetricSkill.ts`, `utils/customMetricImport.ts`, `components/CustomMetricEditorModal.vue`, `components/CustomMetricSkillModal.vue`; modified `types/metrics.ts`, `types/graph.ts`, `stores/metrics.ts`, `stores/graph.ts` (hydration), `workers/metricsWorker.ts`, `components/{GraphCanvas3D,MetricsPanel,DataTablePanel,SidePanel,DetailModal,JavaScriptEditor,TextFormatPanel,TemplatePreviewInline,StylePresetSkillModal}.vue`, `utils/labelFormatter.ts`, `utils/stylePresetSkill.ts`.

**Testing:** backend `tests/test_context_metric_definitions.py` (schemas, memory store, migration, visibility by owner / write share / read share / public / superuser, writes). Frontend: `workers/__tests__/customMetricEvaluate.examples.test.ts` (the A/B/C use-case catalogue verbatim: text A1–A10, numeric B1–B12, boolean C1–C5), `customMetricEvaluate.safety.test.ts` (D1–D9: frozen inputs, coercion, syntax errors, partial item errors, 200k-node perf guard), `customMetricSandbox.test.ts`, `utils/__tests__/customMetricValue.test.ts`, `services/__tests__/customMetricRunner.test.ts` (timeout → terminate + respawn, crash, forged/stale messages, cancel, snapshot), `stores/__tests__/customMetrics.test.ts`, `components/__tests__/{CustomMetricEditorModal,MetricsPanel.custom,CustomMetricSkillModal}.test.ts`, `utils/__tests__/customMetricSkill.test.ts`, `utils/__tests__/customMetricImport.test.ts`, `composables/__tests__/useItemMetrics.test.ts`, `utils/__tests__/metricFormat.test.ts`, extended `metrics`, `labelFormatter`, `stylePresetSkill` tests; e2e `custom-metrics.spec.ts` (writer flow incl. hostile definition, editor Test + Save PUT, reader gating with no worker spawned).
